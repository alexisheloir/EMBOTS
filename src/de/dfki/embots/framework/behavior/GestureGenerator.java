...
import de.dfki.embots.bml.feedback.BML1_0ProgressFeedback;
import de.dfki.embots.bml.feedback.BML1_0SolutionFeedback;
...
//                            _playSender.sendTextMessage("PLAY AUDIO", meta.getTime());
            ...
            }else if (m.getTopicName().equals(EMBOTSConstants.BEHAVIOR_CALLBACK_TYPE)) {

                log.info("[GG INPUT] EMBR feedback");
                // Feedback from the animation engine (currently EMBR)
                processEMBRFeedback(m);
    ...
    private void processGestureFeedback(SEMAINEMessage m)
    {
    ...
    private void processSpeakingFeedback(SEMAINEMessage m) throws DOMException, JMSException
    {
        ...
        if ("start".equals(eventType)) {
            //BML1_0ProgressFeedback fb = new BML1_0ProgressFeedback(BML1_0Feedback.Type.STATUS,
            //        BML1_0ProgressFeedback.Subtype.STARTED,
            //        _bml.getID());
            BML1_0ProgressFeedback fb = new BML1_0ProgressFeedback(BML1_0Feedback.Type.PROGRESS,
                    BML1_0ProgressFeedback.Subtype.STARTED,
                    _bml.getAgent(),
                    _bml.getID(),
                    String.valueOf(meta.getTime()));
            ...
            BML1_0ProgressFeedback fb = new BML1_0ProgressFeedback(BML1_0Feedback.Type.PROGRESS,
                    BML1_0ProgressFeedback.Subtype.FINISHED,
                    _bml.getAgent(),
                    _bml.getID(),
                    String.valueOf(meta.getTime()));
            _bmlFeedbackSender.sendTextMessage(fb.toXML(), meta.getTime());
    ...
    /**
     * Currently not used. process the feedback sent by EMBR
     */
    private void processEMBRFeedback(SEMAINEMessage m) throws DOMException, JMSException
    {
        Document d = ((SEMAINEXMLMessage) m).getDocument();
        Node eventNode = d.getElementsByTagName("event").item(0);
        String eventType = eventNode.getAttributes().getNamedItem("type").getNodeValue();
        if ("sync".equals(eventType)) { // send a sync progress event feedback
        }
    }
    ...
    private void processPhonemeInput(SEMAINEMessage m) throws HeadlessException, Exception
    {
        ...
        // extract ID
        for(int i=0;i<xm.getDocument().getElementsByTagName("bml:speech").getLength();i++){
        Node speechNode = xm.getDocument().getElementsByTagName("bml:speech").item(i);
...
//        System.gc();
        ...
        if (success)
        {
            BMLSyncPoint syncPointsArray[] = _bml.collectSyncPoints();
            int numberOfSyncPoints = syncPointsArray.length;
            if (numberOfSyncPoints > 0)
            {
                double start = syncPointsArray[0].getTime();
                double stop = syncPointsArray[syncPointsArray.length-1].getTime();

                String sstart = Double.toString(start);
                String sstop = Double.toString(stop);

                BML1_0Feedback fbConstraints = new BML1_0SolutionFeedback(BML1_0Feedback.Type.SOLUTION, BML1_0SolutionFeedback.Subtype.BLOCK , _bml.getAgent(), _bml.getID(),"",sstart,sstop);
                try {
        ...
        {
            BML1_0Feedback fbConstraints = new BML1_0WarningFeedback(BML1_0Feedback.Type.WARNING, BML1_0WarningFeedback.Subtype.WARNING04, _bml.getAgent(), _bml.getID(), "Failed to solve temporal constraints");
            try {
                _bmlFeedbackSender.sendTextMessage(fbConstraints.toXML(), meta.getTime());
        ...
        for (BMLBehavior beh : _bml.getBehaviors()) {
            ...
            if (beh instanceof BMLGestureBehavior || beh instanceof BMLHeadBehavior) {
                ...
                if (beh.getLexeme() != null) {
                    ...
                    // BML feedback
                    try {
                        _bmlFeedbackSender.sendTextMessage(new BML1_0SolutionFeedback(BML1_0Feedback.Type.SOLUTION, BML1_0SolutionFeedback.Subtype.BEHAVIORS, beh.getAgent(), beh.getID(), beh.toXML()).toXML(), meta.getTime());
                    } catch (JMSException ex) {
                        Logger.getLogger(GestureGenerator.class.getName()).log(Level.SEVERE, null, ex);
                    }
                } else {
                ...
                if (cons.size() > 0) {
                    ...
                    try {
                        _bmlFeedbackSender.sendTextMessage(new BML1_0WarningFeedback(BML1_0Feedback.Type.WARNING, BML1_0WarningFeedback.Subtype.WARNING04, beh.getAgent(), beh.getID(), "No temporal constraints found: dropping behavior").toXML(), meta.getTime());
                    } catch (JMSException e) {
                        e.printStackTrace();
                    }
                }
