...
set MARYDIR="C:\EMBOTS\Daniel\Programs\Mary_New"

REM set MARYDIR="C:\Users\aheloir\projects\programming\EMBOTSREF\trunk\MARY"
...
if %1%a==a (
  set CONFIG=%BINDIR%..\config\embots_kinect.config
) else (
  set CONFIG=%1%
)
...
java -Xss512k -Xmx1000m %JMS_URL_SETTING% -classpath %DISTDIR%\embots.jar;%JARDIR%\jama.jar;%JARDIR%\jar173_1.0_api.jar;%JARDIR%\xml-text-editor-0.0.2.jar;%JARDIR%\semaine.jar;%JARDIR%\semaine-mary.jar;%JARDIR%\semaine-dialogue.jar;%JARDIR%\JaCoP-3.0-RC1.jar;%JARDIR%\jdom.jar;%JARDIR%\log4j-1.2.15.jar;%JARDIR%\commons-logging-1.1.1.jar;%JARDIR%\activemq-all-5.1.0.jar;%JARDIR%\jgraph.jar;%JARDIR%\VISP.jar;%JARDIR%\affect.jar -Dmary.base="%MARYDIR%" eu.semaine.system.ComponentRunner %CONFIG%
