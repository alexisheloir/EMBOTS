...
if %1%a==a (
  set CONFIG=%BINDIR%..\config\helloWorld.config
) else (
  set CONFIG=%1%
)
...
java -Xss512k -Xmx1000m %JMS_URL_SETTING% -classpath %DISTDIR%\embots.jar;%JARDIR%\jama.jar;%JARDIR%\jar173_1.0_api.jar;%JARDIR%\xml-text-editor-0.0.2.jar;%JARDIR%\semaine.jar;%JARDIR%\semaine-mary.jar;%JARDIR%\semaine-dialogue.jar;%JARDIR%\semaine-examples.jar;%JARDIR%\JaCoP-3.1.jar;%JARDIR%\jdom.jar;%JARDIR%\log4j-1.2.15.jar;%JARDIR%\commons-logging-1.1.1.jar;%JARDIR%\spring-core-3.0.3.RELEASE.jar;%JARDIR%\activemq-all-5.1.0.jar;%JARDIR%\jgraph.jar;%JARDIR%\affect.jar -Dmary.base="%MARYDIR%" eu.semaine.system.ComponentRunner %CONFIG%
